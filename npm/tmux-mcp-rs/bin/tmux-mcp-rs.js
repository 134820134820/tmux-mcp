...
const BIN = "tmux-mcp-rs";
const REPOSITORY = process.env.TMUX_MCP_RS_REPOSITORY || "bnomei/tmux-mcp";
const VERSION = normalizeVersion(process.env.TMUX_MCP_RS_VERSION || packageJson.version);

main().catch((error) => {
  console.error(`tmux-mcp-rs npm wrapper: ${error.message}`);
  process.exit(1);
});
...
async function main() {
  ...
  if (!fs.existsSync(binaryPath)) {
    if (process.env.TMUX_MCP_RS_SKIP_DOWNLOAD === "1") {
      throw new Error(
        "binary not cached and TMUX_MCP_RS_SKIP_DOWNLOAD=1; set TMUX_MCP_RS_LOCAL_BIN or clear SKIP_DOWNLOAD",
      );
    }
...
function cacheDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.LOCALAPPDATA || os.tmpdir(), "tmux-mcp-rs", "npm");
  }
...
async function installRelease(binaryPath, release) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "tmux-mcp-rs-npm-"));
...
function download(url, destination, redirects = 0) {
  return new Promise((resolve, reject) => {
    const request = https.get(
      ...
      {
        headers: {
          "user-agent": "tmux-mcp-rs-npm-wrapper",
        },
      },
      (response) => {
        if (
