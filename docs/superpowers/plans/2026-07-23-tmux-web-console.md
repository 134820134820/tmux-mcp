...
- [ ] Write failing tests for state directory resolution, Gate file behavior,
      record classification, target extraction, and result snapshots.
- [ ] Implement the minimum serializable types and optional HTTP control client.
...
- [ ] Write failing tests proving read-only calls bypass control, mutating calls
      require decisions, rejection does not invoke the route, and tracked command
      terminal events update the same record.
- [ ] Add the explicit `call_tool` wrapper and terminal event forwarding.
- [ ] Verify server/control tests pass.

### Task 3: Local web service

**Files:** create `src/web.rs`; modify `src/main.rs`.

- [ ] Write failing tests for loopback validation, token checks, Gate
      approve/reject, JSONL recovery/retention, and human key grouping.
- [ ] Implement API state, persistence, tmux topology/capture/input, and CLI
      mode selection.
- [ ] Verify web and CLI tests pass.

### Task 4: Single-page UI

**Files:** create `web/index.html`.

- [ ] Add static-contract tests for required controls and safe text rendering.
- [ ] Implement message/interactive switching, pane navigation, activity
      indicators, Gate dialog, operation log, polling, and keyboard mapping.
- [ ] Verify focused API/UI tests and browser behavior.

### Task 5: Documentation, activation, and verification

**Files:** modify `README.md` and local deployment configuration.

- [ ] Document generic, Codex, and Claude Code startup examples.
- [ ] Run format, focused tests, all-target tests, clippy, and release build.
- [ ] Start the hub locally, perform a dedicated-pane smoke test, and update only
      the current Codex tmux MCP entry after backing it up.
- [ ] Review the diff against the approved design and record any baseline-only
      failures.
